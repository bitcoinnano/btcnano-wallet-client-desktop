...
from bitcoinnano.util import profiler, parse_URI, format_time, InvalidPassword, NotEnoughFunds
from bitcoinnano import bitcoin
from bitcoinnano.util import timestamp_to_datetime
...
class HistoryScreen(CScreen):
    ...
    def get_card(self, tx_hash, height, conf, timestamp, value, balance):
        ...
        ri.message = label
        ri.value = value or 0
        amount = self.app.format_amount(value, True) if value is not None else '--'
        ri.amount = amount[:-1] if amount.endswith(".") else amount
...
class SendScreen(CScreen):
    ...
    def set_URI(self, text):
        try:
            ...
            self.app.show_info(_("Not a Bitcoin Nano URI"))
            return
...
class ReceiveScreen(CScreen):
    ...
    def get_URI(self):
        from bitcoinnano.util import create_URI
    ...
    def do_share(self):
        uri = self.get_URI()
        self.app.do_share(uri, _("Share Bitcoin Nano Request"))
